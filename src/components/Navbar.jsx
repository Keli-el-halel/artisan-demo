    ...
    return (
      <div className="row w-100 px-5" style={{height: "100px", backgroundColor:"black", color:"#fff"}}>
        ...
        <div className="col-6 my-auto d-flex justify-content-end" style={{fontSize: "20px"}}>
            {!localStorage.getItem('user') && <div className={showThis === 'Login' ? "navbarLinks navbarLinkActive" : "navbarLinks"} onClick={() => setShowThis('Login')}>Login</div>}
            {!localStorage.getItem('user') && <div className={showThis === 'Sign Up' ? "navbarLinks navbarLinkActive" : "navbarLinks"} onClick={() => setShowThis('Sign Up')}>Sign Up</div>}
